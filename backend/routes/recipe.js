...
router.get("/:id", async (req, res) => {
  try {
    ...
    console.log(rid);
    let recDoc = await getDoc(doc(db, "recipes", rid));

    let temp = [];
    if (!recDoc.exists()) {
      recDoc = await getDocs(collection(db, "edamam-recipes", rid, "reviews"));
      recDoc.forEach((doc) => temp.push(doc.data()));
      return res.status(200).json(temp);
    }

    // Determine if the recipe is an Edamam recipe
    // const isEdamamRecipe = rec.isEdamamRecipe || false;
    const collectionPath = isEdamamRecipe ? "edamam-recipes" : "recipes";

    // Fetch the reviews from the appropriate collection
    let reviews = await getDocs(collection(db, collectionPath, rid, "reviews"));
    ...
    if (reviews) {
      reviews = await Promise.all(
        reviews.docs.map(async (review) => {
          ...
          revData["replies"] = await Promise.all(
            revData.replies.map(async (rep) => {
...
router.get("/reviews/:id", async (req, res) => {
  try {
    ...
    // Fetch the recipe document to check if it's an Edamam recipe
...
router.put("/review/:id", async (req, res) => {
  try {
    ...
    // Check if the review is for an Edamam recipe
    const isEdamamRecipe = data.isEdamamRecipe || false;

    // Add the review to the appropriate collection based on whether it's for an Edamam recipe
    const collectionPath = isEdamamRecipe ? "edamam-recipes" : "recipes";

    // Add the review document
    await addDoc(collection(db, collectionPath, rid, "reviews"), data);
